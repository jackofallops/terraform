...
func (c *Context) planWalk(config *configs.Config, prevRunState *states.State, rootVariables InputValues, opts *PlanOpts) (*plans.Plan, tfdiags.Diagnostics) {
	...
	driftedResources, driftDiags := c.driftedResources(config, prevRunState, priorState)
...
func (c *Context) driftedResources(config *configs.Config, oldState, newState *states.State) ([]*plans.ResourceInstanceChangeSrc, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
